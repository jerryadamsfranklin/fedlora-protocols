...
Non-IID partitions use Dirichlet concentration $\alpha \in \{0.5, 0.1\}$ over client sampling probabilities, without label conditioning. Under severe skew ($\alpha{=}0.1$), some clients may receive empty partitions and therefore contribute no updates; per-seed active client counts are in Appendix~\ref{app:perseed}.
