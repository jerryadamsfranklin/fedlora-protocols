...
\begin{table}[t]
\caption{Dolly-15k replication. TinyLlama-1.1B, IID, CUDA corpus, three seeds, mean $\pm$ one sample standard deviation. Gap is the held-out $\Delta$loss difference against the same dataset's FLoRA baseline, which is the quantity comparable across datasets; held-out levels are not, since base-model losses differ (1.9352 on Alpaca, 2.2608 on Dolly-15k). Methods: FLoRA \citep{wang2024flora}, FedIT \citep{zhang2024federatedgpt}, FFA-LoRA \citep{sun2024ffalora}.}
\label{tab:dolly}
\begin{center}
\footnotesize
\setlength{\tabcolsep}{4pt}
