...
Where prior federated LoRA accounting reports parameter-count ratios or aggregate megabyte totals \citep{zhang2024federatedgpt,sun2024ffalora,wang2024flora,bai2024flexlora,yan2026fedsrd,ramesh2026florist}, the contribution here is the isolation of two quantities that neither form of reporting separates.
...
Those measurements place five published protocols on a single communication-quality frontier with a knee: the first tranche of savings costs far less held-out instruction-following loss per point than the next. ReverseAdaptive sits at that knee and locates it without a phase boundary being specified in advance; the marginal cost of moving past it is roughly five times higher per point saved.

As open-weight LLMs continue to grow in capability and federated deployments expand into privacy-sensitive domains, the communication bottleneck identified by \citet{kairouz2021advances} will only intensify. Reporting what protocols actually transmit, rather than what a parameter count predicts, is a prerequisite for comparing them.
