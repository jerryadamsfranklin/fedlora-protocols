...
The primary contribution is therefore empirical rather than algorithmic. I instrument a bidirectional federated LoRA protocol at the transport layer, counting the bytes on the wire rather than estimating them from parameter counts, and use that instrumentation to place five protocols, three of them from prior work, on a single measured communication-quality frontier at TinyLlama-1.1B scale on Alpaca. Three of the five are then replicated on Dolly-15k and at LLaMA-3.2-3B. The frontier turns out to have a knee: the first 40.5 percentage points of communication savings cost 0.0063 in held-out instruction-following loss, while the next 21.3 points cost roughly five times more per point. That structure is the paper's central finding, and it is not derivable from any parameter-count analysis.

A secondary contribution operationalizes it. ReverseAdaptive is a schedule that locates the knee by monitoring the relative per-round improvement in global training loss against a dimensionless threshold, rather than by fixing a phase boundary in advance. Because the criterion is scale-free, I transferred a single threshold value from a 1.1B-parameter model to a 3B-parameter model without retuning; the IID switch rounds matched exactly across two hardware backends.

I also report results that do not favor the proposed method, because they bound what the measurements support. The four zero-shot benchmarks commonly used in this literature do not discriminate between aggregation protocols at the smaller scale, where every fine-tuned checkpoint scores below the base model; quality conclusions therefore rest on held-out instruction-following loss alone. At 3B, the adaptive and hand-tuned configurations differ by an amount three seeds cannot resolve, and I report this as an effect size rather than as evidence of equivalence. The 55 MB transition cost I measure is a property of this implementation's frozen-parameter harvest rule rather than of phase-switching protocols in general, and I identify the untested alternative that would eliminate it. I also disclose that my own measurement charges the strongest baseline 55 MB it should not pay, and show that correcting it moves the headline ratio from 5.5 to 5.0 while leaving the conclusion unchanged.
